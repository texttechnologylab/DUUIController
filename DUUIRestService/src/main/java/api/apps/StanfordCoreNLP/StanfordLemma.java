...
public class StanfordLemma {
    public static void main(String[] args) throws Exception {
        ...
        HttpServer server = HttpServer.create(new InetSocketAddress("127.0.0.1", 9003), 0);

        server.getAddress();
        server.createContext("/v1/communication_layer", new StanfordLemma.CommunicationLayer());
        server.createContext("/v1/typesystem", new StanfordLemma.TypesystemHandler());
        ...
        public void handle(HttpExchange t) throws IOException {
            try {
                ...
                for (CoreLabel token : document.tokens()) {
                    Lemma lemma = new Lemma(jc, token.beginPosition(), token.endPosition());
