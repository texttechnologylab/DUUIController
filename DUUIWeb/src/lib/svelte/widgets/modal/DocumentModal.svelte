...
<div
	class="section-wrapper space-y-4 container max-w-6xl sticky top-0 max-h-[80vh] !overflow-y-scroll"
>
	<div
		class="font-bold text-2xl p-4 border-b border-surface-200 dark:border-surface-500 text-center flex items-center justify-between sticky top-0 z-10 bg-fancy"
	>
		<p>{document.name}</p>
		{#if document.error}
